...
impl CssLexer : CssLexerMethods {
    ...
    fn parse_css_attribute(c : u8) -> Token {
        ...
        match ch {
          '.' as u8 => return Attr(values::Includes(~"class", self.input_state.parse_ident())),
          '#' as u8 => return Attr(values::Includes(~"id", self.input_state.parse_ident())),
          '[' as u8 => {
            ...
            if ch == ']' as u8 {
                return Attr(values::Exists(move attr_name));
            } else if ch == '=' as u8 {
                let attr_val = self.input_state.parse_ident();
                self.input_state.expect(']' as u8);
                return Attr(values::Exact(move attr_name, move attr_val));
            } else if ch == '~' as u8 {
                self.input_state.expect('=' as u8);
                let attr_val = self.input_state.parse_ident();
                self.input_state.expect(']' as u8);
                return Attr(values::Includes(move attr_name, move attr_val));
            } else if ch == '|' as u8 {
                self.input_state.expect('=' as u8);
                let attr_val = self.input_state.parse_ident();
                self.input_state.expect(']' as u8);
                return Attr(values::StartsWith(move attr_name, move attr_val));
            }
